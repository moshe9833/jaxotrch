...
## SEO conventions
- Every action sets ViewData["Title"] AND ViewData["MetaDescription"] (unique per
  page; case studies use their Summary). Layout composes "<Title> — Jaxo".
- Canonical host lives in appsettings "CanonicalHost" (jaxotech.com). Program.cs
  301-redirects any other host in production (e.g. *.azurewebsites.net, www).
  Layout emits <link rel="canonical"> from the lowercased path, no query string.
- /sitemap.xml is generated by HomeController.Sitemap from case-studies.json —
  new studies appear automatically. wwwroot/robots.txt points to it.
- JSON-LD: ProfessionalService site-wide (_Layout), Service graph on Services,
  BreadcrumbList on case studies, FAQPage on Contact. Keep NAP (name, address,
  phone) identical everywhere: Monroe NY, (845) 662-6703, moshe@jaxotech.com.
...
- Confirm founder name spelling on About ("Moshe Neuman").
